...
if(isset($_POST['send_notif'])){
   ...
   $to  = $_POST['to_type'];
   if($msg != ''){
      mysqli_query($conn,"INSERT INTO `notifications`(from_type,from_id,from_name,to_type,message) VALUES('waiter','$waiter_id','$waiter_name','$to','$msg')");
...
if(isset($_GET['mark_read'])){
   mysqli_query($conn,"UPDATE `notifications` SET is_read=1 WHERE to_type IN('waiter','all') AND is_read=0");
   header('location:waiter_notifications.php'); exit;
}
...
// Fetch inbox for waiter
$notifs = mysqli_query($conn,"SELECT * FROM `notifications` WHERE to_type IN('waiter','all') ORDER BY is_read ASC, created_at DESC");
$unread = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) as c FROM `notifications` WHERE to_type IN('waiter','all') AND is_read=0"))['c'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   <title>Notifications - Waiter - Spice Garden</title>
   ...
   <style>
      ...
      .notif-card.unread { border-color:#6366f1;background:#eef2ff;box-shadow:0 4px 16px rgba(99,102,241,0.15); }
...
<section>
   ...
   <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:2rem;">
      <?php if($unread > 0): ?>
         <a href="waiter_notifications.php?mark_read=1" class="btn"><i class="fas fa-check-double"></i> Mark All Read</a>
      <?php endif; ?>
   ...
   <div style="margin-bottom:2rem;display:flex;gap:.8rem;flex-wrap:wrap;">
      <span style="font-size:1.4rem;font-weight:600;color:#6b7280;align-self:center;">Quick Send to Cook:</span>
      <?php
      $quick = [
         ['to'=>'cook', 'msg'=>'🔔 Table order is ready to be picked up!',         'label'=>'Order Ready?'],
         ['to'=>'cook', 'msg'=>'⚡ Customer is waiting for order, please hurry!',   'label'=>'Hurry Up!'],
         ['to'=>'cook', 'msg'=>'🍽️ New order placed at table, please check!',       'label'=>'New Order'],
         ['to'=>'admin','msg'=>'⚠️ Need admin attention at the floor!',              'label'=>'Call Admin'],
      ];
      foreach($quick as $q){
      ?>
